...
try:
    ...
    check('desktop: experience summary shown above the sections', ev("document.querySelectorAll('#xp-root .xp-summary li').length") == 3)
    check('desktop: web app icons present', ev("document.querySelectorAll('#app-icons .icon').length") >= 3)
    ev("openApp(APPS[0]); openApp(APPS[1])")
    sleep(0.8)
    check('desktop: browser opens with two tabs and an iframe', ev("document.getElementById('browser-window').classList.contains('open') && document.querySelectorAll('.browser-tab').length === 2 && document.querySelectorAll('#browser-frames iframe').length === 2"))
    ev("closeBrowserTab(APPS[1].id)")
    check('desktop: closing a tab keeps the other', ev("document.querySelectorAll('.browser-tab').length === 1 && document.querySelector('.browser-tab.active') !== null"))
    ev("closeWindow('browser-window')")
    sleep(0.3)
    check('desktop: closing the browser removes its frames', ev("document.querySelectorAll('#browser-frames iframe').length === 0"))
    ev("I18N.set('fr')")
    sleep(0.2)
